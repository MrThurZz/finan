...
entrada/saída e controle de "empréstimos a mim mesmo".
...
2. Em **SQL Editor**, cole e execute o conteúdo de [`supabase/schema.sql`](./supabase/schema.sql).
   Isso cria as tabelas (`categories`, `transactions`, `self_loans` — as tabelas `investment_goals`
   e `investment_contributions` também existem no schema mas não são mais usadas pelo app, veja a
   seção 5), os índices, as policies de Row Level Security (`auth.uid() = user_id` em todas as
   tabelas) e um trigger que popula categorias padrão automaticamente quando um usuário se cadastra.
...
src/
  ...
  hooks/        hooks de dados (React Query) por tabela: transactions, categories, self_loans
  lib/          cliente Supabase e helpers de formatação (moeda, data pt-BR)
  pages/        telas: Dashboard, Devo a mim, Categorias, Relatórios
supabase/
...
- **Devo a mim mesmo** — é um registro 100% manual (`self_loans`). Quando você retira dinheiro de um
  investimento para uso pessoal, cria aqui um registro anotando que precisa devolver aquele valor.
- **Metas de investimento removidas** — a tela de Investimentos (metas, aportes/resgates e o gráfico
  de evolução do patrimônio) foi removida do app a pedido do usuário. As tabelas `investment_goals`
  e `investment_contributions` continuam no schema do Supabase (histórico/dados antigos), mas nada
  na interface as usa mais.
